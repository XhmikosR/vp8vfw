...
IF %ERRORLEVEL% NEQ 0 (ECHO Build failed & GOTO END) ELSE (ECHO x86 installer compiled successfully!)

"%InnoSetupPath%\iscc.exe" /Q "setup.iss" /Dx64Build
IF %ERRORLEVEL% NEQ 0 (ECHO Build failed & GOTO END) ELSE (ECHO x64 installer compiled successfully!)
